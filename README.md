...
它的目标不是“绕过审核”，而是把内容风险、平台反馈、误报样本、成功样本和改写经验持续沉淀下来，逐步形成一个可自进化的内容合规与成稿系统。
...
- `数据维护台`：维护词库、反馈日志、误报样本、改写样本、成功样本、生命周期、模型看板和风格画像。
...
data/
  ...
  success-samples.json       成功样本
  note-lifecycle.json        笔记生命周期与发布结果
...
- 成功样本等级：`featured` > `performed` > `passed`。
...
在“风格画像”区域，可以从高权重成功样本生成画像草稿。画像需要人工确认后才会参与正式生成。
...
- 成功样本优先保存真实过审、真实表现好的内容，避免污染风格画像。
- 对两性 / 身体探索赛道，重点观察组合风险：
  - 性相关话题 + 未成年人线索
  - 性相关话题 + 导流
  - 性相关话题 + 步骤化教学
