...
test("sample library frontend wires parse and commit flows for Markdown imports", async () => {
  ...
  assert.match(commitSource, /const reference = readSampleLibraryImportDraftReference\(sourceItem\)/);
  assert.match(commitSource, /const publish = readSampleLibraryImportDraftPublish\(sourceItem\)/);
  assert.match(commitSource, /referenceEnabled: reference\.enabled === true/);
  assert.match(commitSource, /referenceTier: reference\.tier \|\| ""/);
  assert.match(commitSource, /referenceNotes: reference\.notes \|\| ""/);
  assert.match(commitSource, /publishStatus: publish\.status \|\| "not_published"/);
  assert.match(commitSource, /publishedAt: publish\.publishedAt \|\| ""/);
  assert.match(commitSource, /platformReason: publish\.platformReason \|\| ""/);
  assert.match(commitSource, /publishNotes: publish\.notes \|\| ""/);
  assert.match(commitSource, /views: card\.querySelector\('\[name="views"\]'\)\?\.value \|\| "0"/);
