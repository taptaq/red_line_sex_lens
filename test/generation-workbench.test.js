...
test("buildGenerationMessages includes mode, style profile, success samples, and user requirements", () => {
  ...
  assert.match(combined, /标签还是由你自动生成/);
  assert.match(combined, /热门标签/);
  assert.match(combined, /细分标签/);
  assert.match(combined, /尽量别只给那种特别空的大词标签/);
  assert.match(combined, /至少留 1 个更具体的场景标签/);
  assert.match(combined, /别把 3-6 个标签都塞成泛热门词/);
  assert.match(combined, /也别输出一串意思差不多的重复标签/);
  assert.match(combined, /如果已经有“亲密关系”/);
  assert.match(combined, /就别再连着给几个几乎同义的大词/);
  assert.match(combined, /更适合“1 个稍微宽一点的主标签 \+ 2-4 个更细的标签”/);
  assert.match(combined, /别把 3-6 个名额都分配给同一层级的大词/);
  assert.match(combined, /细分标签优先从具体场景、人群阶段、痛点问题、情绪状态或需求目标里提炼/);
  assert.match(combined, /封面文案比标题更短一点，像顺手丢出来的钩子/);
  assert.match(combined, /和标题别只是重复复述/);
  assert.match(combined, /内太空相关元素要自然融进去，整体表达要符合账号主题/);
  assert.match(combined, /最好让标题、封面文案、正文开头里至少两处能接住这个语境/);
