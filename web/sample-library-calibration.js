...
function buildCalibrationPredictionEvidence({ source = {}, analysis = null, rewrite = null, semanticSummary = "" } = {}) {
  const evidenceSamples = [];
  const evidenceSignals = [];
  const sourceKind = String(source?.kind || "source").trim() || "source";
...
export function buildSampleLibraryCalibrationPrediction(source = {}, selectedModels = {}) {
  ...
  const evidence = buildCalibrationPredictionEvidence({
    source,
    analysis,
    rewrite,
    semanticSummary
